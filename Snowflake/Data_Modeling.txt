...
-- SUM_QTY_MTH: total quantity sold by customer per month
-- SUM_AMT_MTH: total sales amount sold by customer per month
-- AVG_QTY_YR: average quantity sold by customer per year
-- AVG_AMT_YR: average sales amount sold by customer per year
-- MEMBERSHIP_STATUS: customer membership status based on total amount spend per year
-- PROM_FLG_MTH: customer promotion eligibility based on monthly spends

-- 2. Data Model
-- Weekly_Sales_Inventory Fact Table
-- Facts: sum_qty_wk, sum_amt_wk, sum_profit_wk, avg_qty_dy, inv_on_hand_qty_wk, wks_sply, low_stock_flg_wk

-- Monthly_Customer_Program Fact Table
-- Facts: SUM_QTY_MTH, SUM_AMT_MTH, AVG_QTY_YR, AVG_AMT_YR, MEMBERSHIP_STATUS, PROM_FLG_MTH

-- Dimensions: 
-- Date_Dim, Item_Dim, Warehouse_Dim &
-- Customer_Dim
-- Customer
